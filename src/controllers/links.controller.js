...
linksCtrl.renderLinks = async (req, res) => {
    const links = await pool.query('SELECT * FROM links WHERE user_id = 28', [req.user.id]);
    res.render('links/list', { links });
}

linksCtrl.renderLinks_admin = async (req, res) => {
    const links_admin = await pool.query('SELECT * FROM links WHERE user_id = 28', [req.user.id]);
